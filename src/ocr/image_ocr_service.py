...
class ImageOcrService(IOcrService):

    def perform_request(
        self, images: list[BinaryIO], prefer_language: Optional[list[str]] = None
    ) -> list[OcrServiceResponseItemInSinglePage]:

        images = [self._binaryio_to_pillow_image(image) for image in images]
    ...
    @debug_timer
    def _binaryio_to_pillow_image(self, binaryio: BinaryIO) -> Image.Image:
        return Image.open(binaryio)
